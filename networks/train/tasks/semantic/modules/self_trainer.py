...
class Trainer():
    ...
    def train_epoch(self, train_loader, model, model2, optimizer, optimizer2, epoch, evaluator, scheduler, scheduler2, color_fn, report=10,
        ...
        for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, proj_range, _, _, _, _, _, _) in enumerate(train_loader):
            ...
            binary_mask = torch.bernoulli(torch.full((B, 1, H, W), 0.5)).bool() # 1 for trainable points
            if self.gpu: binary_mask = binary_mask.cuda()
            
            proj_range = torch.clamp(in_vol[:, [0], ...].detach(), min=1.0, max=80.0)
            intensity = in_vol[:, [4], ...] * proj_range**2
            ...
            mean_dist = torch.mean(closest_n, dim=1, keepdim=True)
            density = mean_dist / proj_range
            di = torch.cat((density, intensity), dim=1)
            di = di * valid_mask
            ...
            with torch.no_grad():
                evaluator.reset()
                argmax = ((p_difficulty > -0.15)*valid_mask).long() + 1
    ...
    def validate(self, val_loader, model, model2, evaluator, class_func, color_fn, save_scans):
        losses = AverageMeter()
        jaccs = AverageMeter()
        wces = AverageMeter()
        acc = AverageMeter()
