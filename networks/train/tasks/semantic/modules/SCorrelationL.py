...
class NeighborEncoderBlock(nn.Module):
    ...
    def Neighbor_conv(self, inputs):
        ...
        unfold_inputs = F.unfold(proj_range,
                            kernel_size=(self.search, self.search),
                            padding=(pad, pad))
        ...
        knn_values, knn_idx = difference.topk(self.knn_nr, dim=1, largest=False) 
        unfold_inputs = torch.gather(input=unfold_inputs, dim=1, index=knn_idx) 
...
class SCorrL(nn.Module):
    def __init__(self, nclasses, params):
        super(SCorrL, self).__init__()
        
        self.NeighborEncoderBlock = NeighborEncoderBlock(1, 32)
    ...
    def forward(self, x):
        NeighborEncoderBlock = self.NeighborEncoderBlock(x)
