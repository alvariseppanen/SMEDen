...
class KNNConvBlock(nn.Module):
    ...
    def KNN_conv(self, inputs, binary_mask):
        ...
        unfold_range = torch.zeros_like()
        knn_values = torch.zeros_like()
        ...
        for echo in range(self.N_echoes):
            ...
            knn_values = torch.cat((knn_values, n_knn_values.view(B, self.knn_nr, H, W).unsqueeze(dim=1)), dim=1)
    ...
    def forward(self, x, binary_mask):
        x, knn_values = self.KNN_conv(x, binary_mask)
        x = self.act1(x)
...
class Teacher(nn.Module):
    def __init__(self, nclasses, params, N_echoes=3):
        ...
        self.KNNBlock = KNNConvBlock(2, 2 * 32, N_echoes)
    ...
    def forward(self, x, binary_mask):
        KNNBlock, knn_values = self.KNNBlock(x, binary_mask)
