...
class MultiEchoNeighborBlock(nn.Module):
    ...
    def __init__(self, in_chans=3, stem_size=32, n_echoes=2):
        ...
        kernel_knn_size = 3 # 3
        self.search = 7 # 5
        self.range_weight = Parameter(torch.Tensor(in_chans, stem_size, kernel_knn_size*kernel_knn_size*n_echoes))
    ...
    def Neighbor_conv(self, inputs, binary_mask):
        ...
        first_unfold_range = F.unfold(first_range * ~binary_mask,
                                kernel_size=(self.search, self.search),
                                padding=(pad, pad))

        first_unfold_points = F.unfold(first_points * ~binary_mask,
                            kernel_size=(self.search, self.search),
                            padding=(pad, pad))
        ...
        knn_values = torch.zeros((B, 0, H*W)).cuda()
        ...
        for echo in range(self.n_echoes):
            ...
            n_unfold_range[n_knn_values >  0.05 * n_range_self] = 0 # radius cutoff
            unfold_range = torch.cat((unfold_range, n_unfold_range, n_knn_index), dim=1)
            knn_values = torch.cat((knn_values, n_knn_values), dim=1)
        
        knn_values = knn_values.view(B, self.n_echoes, self.knn_nr, H, W)

        output = torch.matmul(self.range_weight.view(self.out_channels, -1), unfold_range).view(B, self.out_channels, H, W)
        
        return output, knn_values
...
class CoorL(nn.Module):
    def __init__(self, nclasses, params, n_echoes=2):
        super(CoorL, self).__init__()
        
        self.MultiEchoNeighborBlock = MultiEchoNeighborBlock(2, 2 * 32, n_echoes)
    ...
    def forward(self, x, binary_mask):
        MultiEchoNeighborBlock, knn_values = self.MultiEchoNeighborBlock(x, binary_mask)
