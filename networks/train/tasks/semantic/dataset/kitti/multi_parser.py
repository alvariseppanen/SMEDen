...
from common.multi_laserscan import LaserScan, SemLaserScan
import torch
import random
import numpy as np
...
class SemanticKitti(Dataset):
  ...
  def __init__(self, root,    # directory where data is
    ...
    for seq in self.sequences:
      ...
      # get paths for each
      #clean_scan_path = os.path.join(self.root, seq, "velodyne")
      #label_path = os.path.join(self.root, seq, "labels")
      scan_path = os.path.join(self.root, seq, "snow_velodyne")
      label_path = os.path.join(self.root, seq, "snow_labels")
      
      # get files
      #clean_scan_files = [os.path.join(dp, f) for dp, dn, fn in os.walk(
      #    os.path.expanduser(clean_scan_path)) for f in fn if is_scan(f)]
  ...
  def __getitem__(self, dataset_index):
    ...
    for index in range(start_index, start_index + 1):
      ...
      proj_range = torch.from_numpy(scan.stack_proj_range).clone()
      proj_xyz = torch.from_numpy(scan.stack_proj_xyz).clone()
      proj_remission = torch.from_numpy(scan.stack_proj_remission).clone()
      proj_mask = torch.from_numpy(scan.stack_proj_mask)

      #proj_idx = torch.from_numpy(scan.stack_proj_idx).clone()
      stack_order = torch.from_numpy(scan.stack_order.copy())
      ...
      proj_z = torch.full([self.max_points], -1, dtype=torch.long)
      proj_z[:unproj_n_points] = torch.from_numpy(scan.proj_z)

      proj_range = proj_range * proj_mask.float()
      proj_xyz = proj_xyz * proj_mask[...,None].float()
      proj_remission = proj_remission * proj_mask.float()
      proj_xyz = proj_xyz.flatten(start_dim=2)
        
      proj = torch.cat(
          [proj_range.clone().permute(2, 0, 1),
           proj_xyz.clone().permute(2, 0, 1),
           proj_remission.clone().permute(2, 0, 1)])
    ...
    # return
    return proj_full, proj_mask, proj_labels, unproj_labels, path_seq, path_name, proj_x, proj_y, proj_z, proj_range, \
           unproj_range, unproj_xyz, proj_remission, unproj_remissions, unproj_n_points, stack_order
  ...
  def __init__(self,
    ...
    if self.split == 'train':
      self.train_dataset = SemanticKitti(root=self.root,
                                         ...
                                         transform=False, # set to True to augment the data
                                         gt=self.gt)
